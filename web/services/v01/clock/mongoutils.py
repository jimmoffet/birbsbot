...
def usajobs():
    ...
    # msg = "Hello, World! I've been updated!"
    # response = slackAPISendMessage(msg, "usajobs")
    # return True

    ### MAKE THIS USE THE LAMBDA
    ...
    search_phrases = [
        ...
        ('"chief innovation"', 16),
        ('"chief product"', 16),
        ('"director innovation"', 16),
        ('"director product"', 16),
        ('"chief digital"', 16),
        ('"chief technology"', 16),
        ('"chief information"', 16),
        ('"chief user"', 16),
        ('"emerging technology"', 16),
        ('"emerging technologies"', 16),
        ('"open-source"', 16),
        ('"open source"', 16),
        ('"director of digital"', 16),
        ('"cloud"', 16),
        ('"product roadmap"', 13),
        ('"product manager"', 13),
    ...
    for search_phrase, grade in search_phrases:
        try:
            ...
            data = {"grade": grade, "search_phrase": search_phrase}

            headers = {}

            URL = "https://oa6rf3guryvj3qln5tb4mlmyim0onemc.lambda-url.us-west-2.on.aws/"

            r = requests.post(URL, headers=headers, json=data)
            if r.status_code == 200:
                log.warning(
                    "search_phrase: " + search_phrase + ", status_code: " + str(r.status_code) + ", " + str(r.json())
                )
            else:
